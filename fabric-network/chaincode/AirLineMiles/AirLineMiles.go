...
func purchaseFlight(stub shim.ChaincodeStubInterface, args []string) (string, error) {
	...
	if flightCost <= 0 {
		return "", fmt.Errorf("Error getting flight price")
	}
	if userAirlineMiles <= 0 {
		return "", fmt.Errorf("Error getting user airline miles")
	}
	if userAirlineMiles >= flightCost {
		...
		// Ugly work-around because you cannot assign to map indexes directly :(
